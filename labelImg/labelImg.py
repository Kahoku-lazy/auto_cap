...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        self.actions = struct(save=save, save_format=save_format, saveAs=saveAs, open=open, close=close,
                              ...
                              fileMenuActions=(
                                  open, opendir, save, saveAs, close, resetAll, quit, duplicate_xml, batch_duplicate,
                                  label_pruning, folder_info, data_agument),
                              beginner=(), advanced=(),
        ...
        addActions(self.menus.video,
                   (extract_video, extract_stream, None, batch_resize_img, merge_video, None, annotation_video))
                ...
                if i == 0:
                    ...
                    if os.path.exists(save_path):
                        ...
                        for key in xml_info.keys():
                            root.find(key).text = xml_info[key]
                        tree.write(save_path)
            QMessageBox.information(self, u'Done!',
                                    u'batch duplicate xml file succeed, you can procesing other job now.')
        ...
        if self.question_2(prune_list):
            try:
                ...
                if len(imglist) != len(xmllist):
                    QMessageBox.information(self, u'Wrong!',
                                            u'file list length are different({0}/{1}), please check.'.format(
                                                len(imglist), len(xmllist)))
                else:
                    for item in xmllist:
                        ...
                        if not keep:
                            ...
                            os.remove(
                                os.path.join(os.path.abspath(os.path.dirname(self.filePath)), item[0:-4]) + '.jpg')
    ...
    def remove_extra_img_xml(self):
        ...
        try:
            ...
            imglist = os.listdir(img_folder_path)
            xmllist = os.listdir(xml_folder_path)
            QMessageBox.information(self, u'Info!',
                                    u'done, now have {} imgs, and {} xmls'.format(len(imglist), len(xmllist)))
        except Exception as e:
            QMessageBox.information(self, u'Sorry!', u'something is wrong. ({})'.format(e))
    ...
    def show_folder_infor(self):
        ...
        try:
            imglist = os.listdir(os.path.dirname(self.filePath))
            xmllist = os.listdir(self.defaultSaveDir)
            QMessageBox.information(self, u'Haa',
                                    u'img path: {0}\nimg nums: {1} imgs\nxml path: {2}\nxml nums: {3} xmls'.format(
                                        os.path.dirname(self.filePath), len(imglist), self.defaultSaveDir,
                                        len(xmllist)))
    ...
    def extract_video(self):
        ...
        try:
            ...
            if not video_path:
                return
            save_path = os.path.join(os.path.dirname(os.path.abspath(video_path)),
                                     os.path.realpath(video_path).split('.')[0])
    ...
    def extract_stream(self):
        ...
        try:
            ...
            max_frame, ok = QInputDialog.getInt(self, 'Int Input Dialog',
                                                "Input max save number, process will end after save cetain number imgs",
                                                value=10)
            if not ok:
                return
    ...
    def merge_video(self):
        ...
        try:
            ...
            repeat_time, ok = QInputDialog.getInt(self, 'Int Input Dialog',
                                                  "Input each img's repeat times(the bigger, the slower), usually set 1",
                                                  value=1)
            if not ok:
                return
    ...
    def annotation_video(self):
        ...
        try:
            ...
            if not videoPath:
                videoPath = 0
            save_gap, ok = QInputDialog.getInt(self, 'Integer input dialog',
                                               'input save gap, img will saved by this frenquency :', value=25)
            if not ok:
                return
            img_size, ok = QInputDialog.getInt(self, 'Integer input dialog',
                                               'input img size, img resized ti this shape by height:', value=900)
            if not ok:
                return
            ...
            def init_multiTracker(frame):
                ...
                while True:
                    ...
                    bbox = cv2.selectROI(
                        "draw box and press 'SPACE' to affirm, Press 'q' to quit draw box and start tracking-labeling",
                        frame)
            ...
            while cap.isOpened():
                ...
                for i, newbox in enumerate(boxes):
                    ...
                    cv2.rectangle(draw, p1, (int(newbox[0]) + t_size[0] + 3, int(newbox[1]) + t_size[1] + 6), colors[i],
                                  -1)
                    cv2.putText(draw, info, (int(newbox[0]) + 1, int(newbox[1]) + t_size[1] + 2),
                ...
                if index % save_gap == 0:
                    ...
                    for i, newbox in enumerate(boxes):
                        ...
                        temp_obj.find('bndbox').find('xmax').text = str(
                            int(weight_K * newbox[0] + weight_K * newbox[2]))
                        temp_obj.find('bndbox').find('ymax').text = str(
                            int(height_K * newbox[1] + height_K * newbox[3]))
    ...
    def yolov5_auto_labeling(self):
        try:
            ...
            if len(names) == 1:
                ...
                needed_labels = easygui.multchoicebox(msg="select labels you want auto-labeing?", title="Setect labels",
                                                      choices=tuple(names, ))
            # set imsize
    ...
    def data_auto_agument(self):
        ...
        try:
            ...
            if len(imglist) != len(xmllist):
                ...
                magnification, OK = QInputDialog.getInt(self, 'Integer input dialog',
                                                        'input agument magnification(1~4):', value=4)
                if OK:
                    ...
                    imglist1 = img_Temp[0:one_step];
                    xmllist1 = xml_Temp[0:one_step]
                    imglist2 = img_Temp[one_step:2 * one_step];
                    xmllist2 = xml_Temp[one_step:2 * one_step]
                    imglist3 = img_Temp[2 * one_step:3 * one_step];
                    xmllist3 = xml_Temp[2 * one_step:3 * one_step]
                    imglist4 = img_Temp[3 * one_step:];
                    xmllist4 = xml_Temp[3 * one_step:]
                    progress = QProgressDialog(self)
                    progress.setWindowTitle(u"Waiting")
